...
export default function StateAdminDashboard() {
  ...
  useEffect(() => {
    ...
    if (userProfile.role !== UserRole.STATE_ADMIN) { router.push('/'); return; }
    loadDashboardData();
  ...
  if (!userProfile || userProfile.role !== UserRole.STATE_ADMIN) return null;
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 p-6">
      <div className="max-w-7xl mx-auto mb-8">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600 mt-1">{userProfile.name} • <span className="font-semibold">{userProfile.state}</span></p>
          </div>
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-600">Hospitals in {userProfile.state}</CardTitle></CardHeader><CardContent><div className="text-3xl font-bold">{stats.totalHospitals}</div></CardContent></Card>
        ...
        <Card>
          <CardHeader><CardTitle>Hospitals in {userProfile.state}</CardTitle></CardHeader>
          <CardContent>
            <Table>
              <TableHeader><TableRow><TableHead>Hospital Name</TableHead><TableHead>Address</TableHead><TableHead>Total Beds</TableHead><TableHead>NICU Beds</TableHead></TableRow></TableHeader>
              <TableBody>
                {hospitals.map((hospital) => (
                  <TableRow key={hospital.id}><TableCell className="font-medium">{hospital.name}</TableCell><TableCell>{hospital.address}</TableCell><TableCell>{hospital.totalBeds}</TableCell><TableCell>{hospital.nicuBeds}</TableCell></TableRow>
                ))}
              </TableBody>
            </Table>
          </CardContent>
